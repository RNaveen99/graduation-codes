...
//sorting processes in Descending order w.r.t Priority
...
void tempDisplayProcess(Process p, const int & oldTime, const int & newTime) {
	...
	cout << "\n|----------| " << endl;
	cout << "|          | " << endl;
	cout << "|    P" << p.processId << "    | " << endl;
	cout << "|----------| " << endl;
	cout << oldTime << "         " << newTime << "  ";
...
void displayProcess(Process * p, const int & n, const double & avgTAT, const double & avgWT) {
	cout << "\n\nP_id\t Priority \t Arrival Time\t Burst Time\t starting time\t  Completion Time\t TurnAround Time\t Waiting Time";
	for (int i = 0; i < n; i++) {
